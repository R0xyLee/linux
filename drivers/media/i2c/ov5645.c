...
static int ov5645_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	int ret;
	dbg("ov5645_s_stream [%d]\n", enable);

	if (enable) {
		if (!ov5645->current_mode) {
			dev_err(ov5645->dev, "current mode is empty, please set format first\n");
			return -EINVAL;
		}
...
static int ov5645_probe(struct i2c_client *client)
{
	...
	memset(ov5645, 0, sizeof(struct ov5645));
	...
	ret = v4l2_fwnode_endpoint_parse(of_fwnode_handle(endpoint), &ov5645->ep);
