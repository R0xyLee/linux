...
static void imx8mp_ldb_encoder_enable(struct drm_encoder *encoder)
{
	...
	if (imx8mp_ldb_ch == &imx8mp_ldb->channel[0] || ldb->dual || ldb->duplicate) {
		ldb->ldb_ctrl &= ~LDB_CH0_MODE_EN_MASK;
		ldb->ldb_ctrl |= LDB_CH0_MODE_EN_TO_DI0;
	}
	if (imx8mp_ldb_ch == &imx8mp_ldb->channel[1] || ldb->dual || ldb->duplicate) {
		ldb->ldb_ctrl &= ~LDB_CH1_MODE_EN_MASK;
		ldb->ldb_ctrl |= (ldb->dual || ldb->duplicate) ?
				LDB_CH1_MODE_EN_TO_DI0 : LDB_CH1_MODE_EN_TO_DI1;
	}

	if (ldb->dual || ldb->duplicate) {
