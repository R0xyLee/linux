...
static void ldb_ch_set_bus_format(struct ldb_channel *ldb_ch, u32 bus_format)
{
	...
	case MEDIA_BUS_FMT_RGB888_1X7X4_SPWG:
		if (ldb_ch->chno == 0 || ldb->dual || ldb->duplicate)
			ldb->ldb_ctrl |= LDB_DATA_WIDTH_CH0_24;
		if (ldb_ch->chno == 1 || ldb->dual || ldb->duplicate)
				...
				const struct drm_display_mode *adjusted_mode)
{
	...
	/* FIXME - duplicate mode connections DI0 --> CH0, DI0 --> CH1 */
...
static void ldb_bridge_disable(struct drm_bridge *bridge)
{
	...
	if (ldb_ch == ldb->channel[0] || ldb->dual || ldb->duplicate)
		ldb->ldb_ctrl &= ~LDB_CH0_MODE_EN_MASK;
	if (ldb_ch == ldb->channel[1] || ldb->dual || ldb->duplicate)
...
int ldb_bind(struct ldb *ldb, struct drm_encoder **encoder)
{
	...
	ldb->duplicate = false;
	ldb->duplicate = of_property_read_bool(np, "fsl,duplicate-channel");
	ldb->dual = of_property_read_bool(np, "fsl,dual-channel");
	if (ldb->dual)
		ldb->ldb_ctrl |= LDB_SPLIT_MODE_EN;

	if (ldb->duplicate && ldb->dual) {
		dev_err(dev, "unsupport both duplicate and dual, set dual mode\n");
		ldb->duplicate = false;
	}
	...
	for_each_child_of_node(np, child) {
		...
		if (ldb->duplicate && i > 0) {
			dev_warn(dev, "duplicate-channel mode, ignoring second output\n");
			continue;
		}
