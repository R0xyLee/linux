				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	if (codec_priv->pll_id && priv->card_type == CARD_NAU88C22){
		if (priv->sample_format == SNDRV_PCM_FORMAT_S24_LE ||
				priv->sample_format == SNDRV_PCM_FORMAT_S20_3LE)
			pll_out = priv->sample_rate * 384;
		else
...
static int fsl_asoc_card_probe(struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "fsl,imx-audio-cs42888")) {
		...
		priv->card_type = CARD_WM8958;
	} else if (of_device_is_compatible(np, "fsl,imx-audio-nau88c22")) {
		codec_dai_name = "nau8822-hifi";
		priv->dai_fmt |= SND_SOC_DAIFMT_CBM_CFM;
		priv->card_type = CARD_NAU88C22;
		priv->codec_priv.pll_id = 1;
		priv->codec_priv.mclk_freq = 12288000;
	} else {
